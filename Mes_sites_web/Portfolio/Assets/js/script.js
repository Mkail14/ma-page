...
if (map && slides.length > 0) {
  let interval;
  map.addEventListener("mouseenter", () => { interval = setInterval(changeSlide, 2000); });
  map.addEventListener("mouseleave", () => { clearInterval(interval); });
}
...
function initBackgroundEffect() {
  ...
  const gridCache = { canvas: null, signature: "" };
  let spotlightRaf = 0;
  let pointerInside = false;
  let pointerClientX = 0;
  let pointerClientY = 0;

  function canAnimate() { return state.isInViewport && state.isDocumentVisible; }
  function getGridColor() { return { r: 0, g: 255, b: 204 }; }
  ...
  function scheduleSpotlight() {
    ...
    spotlightRaf = requestAnimationFrame(() => {
      spotlightRaf = 0;
      if (!pointerInside) { spotlight.style.transform = `translate(${hiddenOffset}px, ${hiddenOffset}px)`; return; }
  ...
  function computeGridInfo() {
    ...
    return { width, height, cell, numCols: Math.ceil(width / cell), numRows: Math.ceil(height / cell) };
  }
  ...
  function getCachedGrid(width, height, cell, numCols, numRows) {
    ...
    ctx.beginPath();
    for (let c = 0; c <= numCols; c++) { ctx.moveTo(c * cell, 0); ctx.lineTo(c * cell, numRows * cell); }
    for (let ri = 0; ri <= numRows; ri++) { ctx.moveTo(0, ri * cell); ctx.lineTo(numCols * cell, ri * cell); }
    ctx.stroke();
  ...
  function generateNeighbors(row, col, numRows, numCols) {
    ...
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (Math.random() < neighborChance) out.push({ row: r, col: c, opacity: 255 });
      }
    }
    return out;
  }
  ...
  function renderFrame() {
    ...
    for (const dirty of state.dirtyCells) {
      const x = dirty.col * cell - 1, y = dirty.row * cell - 1, w = cell + 2, h = cell + 2;
    ...
    if (mouseX >= 0 && mouseX <= width && mouseY >= 0 && mouseY <= height) {
      ...
      if (row !== state.currentRow || col !== state.currentCol) {
        state.currentRow = row; state.currentCol = col;
    ...
    if (state.neighbors.length > 0) state.animId = requestAnimationFrame(renderFrame);
  }
  ...
  function onPointerLeave() {
    pointerInside = false;
    state.mouseX = -1000; state.mouseY = -1000;
    state.currentRow = -2; state.currentCol = -2;
    scheduleSpotlight();
    refreshAndRender();
  }
  ...
  function onPointerDown(e) {
    ...
    if (target && target !== bgEffect && target.closest('a,button,input,textarea,select,[role="button"]')) return;
    const rect = bgEffect.getBoundingClientRect();
  ...
  function onVisibilityChange() {
    ...
    if (!state.isDocumentVisible) { stopAnimation(); return; }
    refreshAndRender();
  }
  ...
  function onViewportChange(entries) {
    ...
    if (!state.isInViewport) { stopAnimation(); return; }
    refreshAndRender();
  }
  ...
  const resizeObserver = typeof ResizeObserver === "undefined" ? null :
    new ResizeObserver(() => { resizeCanvasIfNeeded(); refreshAndRender(); });
  ...
  if (!resizeObserver) {
    window.addEventListener("resize", () => { resizeCanvasIfNeeded(); refreshAndRender(); });
  } else {
    resizeObserver.observe(bgEffect);
  }

  if (intersectionObserver) intersectionObserver.observe(bgEffect);
